...
const Waiting = () => {
    ...
    async function getBookings(page, studioNumber, selectedField, searchText) {
        setLoading(true)
        try {
            const res = await userRequest.get(`/booking/waiting/history/admin?page=${page}&limit=10&manage=true&studio=${studioNumber}&${selectedField}=${searchText}`)
            setLoading(false)
            return res
        } catch (error) {
    ...
    const handleSlotDataCSVDownload = async () => {
        try {
            const res = await userRequest.get(`/booking/waiting/history/admin?studio=all&typeOfRequest=downloadCsv`,{ responseType: 'blob' })
            fileDownload(res.data, "StudioSlotWaitingReport.xlsx")
        } catch (error) {
            console.log(error.message)
        }
                                                    ...
                                                    bookings?.map((booking,index) => {
                                                        return (
                                                                ...
                                                                <tr key={booking?.bookings?._id} >
                                                                    ...
                                                                    <Tooltip title={booking?.bookings?.program} color='grey' key={booking?.bookings?._id} placement="left">
